...
namespace Models
{
    ...
    {
        [FormerlySerializedAs("_type")] [Tooltip("Enum of type DevelopingCountrySociety, EmergingCountrySociety or IndustrialCountrySociety")]
        public DemographyType type;
        [FormerlySerializedAs("_initialPopulationCount")] public int initialPopulationCount;
        ...
        public List<int> CreateAgeDistributionTemplate()
        {
            ...
            {
                int paritalCount = (int) (initialPopulationCount * distributionPerDecade[i] / 100);
                for (int j = 0; j < paritalCount; j++)
                {
                    int min = i * 10;
                    int max = (i + 1) * 10;
        ...
        public List<double> CreateQualificationStructure()
        {
            if (type == DemographyType.IndustrialCountrySociety)
            {
                return new double[] {16, 64, 20}.ToList();
            }
